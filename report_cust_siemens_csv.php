...
header("Content-Disposition: attachment; filename=importshipment-proccess$datenow.xls");
...
<table id="example1"  class="table table-bordered table-striped">
  ...
  <tbody>
    ...
    $result = mysqli_query($con,"SELECT * FROM tb_master_impor 
      ...
      WHERE tb_master_impor.rcd_type = 'import' AND tb_master_impor.rcd_create_month='$monthnow' AND tb_master_impor.rcd_create_year='$yearnow' AND tb_master_impor.rcd_cnee LIKE '%siemens%' ");
    ...
    if(mysqli_num_rows($result)>0){ 
      ...
      while($row = mysqli_fetch_array($result))
      {
        ...
        /* CALCULATE LEADTIME */
          $Lead0 = strtotime($dlv1['dlv_date'])-strtotime($row['rcd_ata']);
          $LeadSql   = floor($Lead0 / (60 * 60 * 24));
          echo "<td>" . $LeadSql . "</td>";
        /* CALCULATE LEADTIME - END */

        /* KPI LEADTIME */
        if ($dlv1['dlv_date'] == "0000-00-00") {
          echo "<th>" . "DLVDATE NOT FOUND!" . "</th>";
        } else {
          if ($row['rcd_mot'] == 'FCL') {
            if ($LeadSql <= 3) {
              echo "<th>" . "ACHIEVED" . "</th>";
            } else {
              echo "<th>" . "NOT ACHIEVED" . "</th>";
            }
          } elseif ($row['rcd_mot'] == 'LCL') {
            if ($LeadSql <= 5) {
              echo "<th>" . "ACHIEVED" . "</th>";
            } else {
              echo "<th>" . "NOT ACHIEVED" . "</th>";
